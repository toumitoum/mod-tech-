...
export default function TrustSections() {
  return (
    <>
      <section aria-labelledby="industries-heading" className="bg-[#0b1116] py-[clamp(72px,8vw,108px)]">
        <div className="mod-container overflow-hidden rounded-[32px] border border-white/12 bg-[#081015] shadow-[0_26px_70px_rgba(0,0,0,0.28)] lg:grid lg:grid-cols-[0.9fr_1.1fr]">
          <motion.div
            variants={fadeUp}
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            className="relative overflow-hidden bg-[#0b2022] px-7 py-10 sm:px-10 sm:py-14"
          >
            ...
            <div className="relative z-10">
              ...
              <h2 id="industries-heading" className="mb-6 max-w-lg text-[clamp(2rem,3.8vw,3.5rem)] font-light leading-[1.02] tracking-[-0.04em] text-white">Des solutions adaptées aux sites qui exigent de la fiabilité.</h2>
          ...
          <div className="grid gap-px bg-white/10 sm:grid-cols-2">
            {industries.map(({ icon: Icon, label }, index) => (
              <motion.div
                ...
                className="group flex min-h-32 items-center gap-4 bg-[#081015] p-6 transition-colors duration-300 hover:bg-[#0d1b20]"
              >
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.035] transition-all duration-300 group-hover:border-[#56ddb4]/45 group-hover:bg-[#56ddb4]/10">
                  <Icon className="h-5 w-5 text-slate-300 transition-colors duration-300 group-hover:text-[#70e7c1]" strokeWidth={1.8} />
                </div>
                <div>
                  <span className="block text-base font-semibold tracking-[-0.02em] text-white">{label}</span>
                  <span aria-hidden="true" className="mt-1 block font-mono text-[10px] tracking-[0.16em] text-white/35">0{index + 1}</span>
                </div>
              </motion.div>
            ))}
          </div>
