...
export default function ProductPage() {
  ...
  const finalPrice = product ? Math.round(product.price * (1 - discPct / 100)) : 0;
  ...
  const addToCart = () => {
    ...
    setCart(prev => {
      ...
      return ex
        ...
        : [...prev, { ...product, qty, selectedColor: selColor, selectedSize: selSize }];
    });
  ...
  const sendOrder = async () => {
    ...
    try { await fetch("/api/google-sheet", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(orderData) }); } catch {}
    try { await fetch("/api/send-order-email", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ order: orderData }) }); } catch {}
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <button onClick={() => { setSuccess(false); setForm({ name:"", phone:"", email:"", wilaya:"", commune:"", address:"", notes:"" }); }}
              ...
              <div className="bg-white border border-slate-100 rounded-2xl p-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {product.colors.map(c => (
                    <button key={c.name} onClick={() => setSelColor(selColor === c.name ? "" : c.name)}
                      title={c.name}
              ...
              <div className="bg-white border border-slate-100 rounded-2xl p-4">
                ...
                <div className="flex gap-2 flex-wrap">
                  {product.sizes.map(sz => (
                    <button key={sz} onClick={() => setSelSize(selSize === sz ? "" : sz)}
              ...
              {!cart.length && !checkout ? (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button onClick={() => removeItem(item.id, item.selectedColor, item.selectedSize)}
                ...
                <>
                  <div className="flex-1 overflow-y-auto px-5 md:px-6 py-4 space-y-4">
                    ...
                    <Field label="Nom complet *"><input  value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className={inputCls} /></Field>
                    <Field label="Téléphone *"><input type="tel" placeholder="ex. 06 XX XX XX XX" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} className={inputCls} /></Field>
                    <Field label="Email"><input type="email" placeholder="ex@email.com" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className={inputCls} /></Field>
                    <Field label="Wilaya *">
