...
export default function LoginPage() {
  ...
  return (
    <div dir="rtl" style={{minHeight:"100vh",background:bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',Tahoma,Arial,sans-serif",transition:"background 0.3s",position:"relative"}}>
      ...
      <button onClick={()=>{const nd=!dark;setDark(nd);localStorage.setItem("mt_theme",nd?"dark":"light");}}
        style={{position:"absolute",top:20,left:20,background:dark?"rgba(51,65,85,0.4)":"rgba(226,232,240,0.8)",border:"1px solid "+brd,borderRadius:10,padding:"8px 14px",cursor:"pointer",color:text,fontSize:18}}>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            ...
            <div style={{position:"relative"}}>
              ...
              <button type="button" onClick={()=>setShow(!show)}
                style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",background:"transparent",border:"none",cursor:"pointer",color:sub,fontSize:17,padding:0}}>
                {show?"N":"Y"}
              </button>
            </div>
          </div>
